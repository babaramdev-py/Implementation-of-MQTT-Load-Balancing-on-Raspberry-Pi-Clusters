...
def execute_cpp_code(code):
    with open("temp3.cpp", "w") as file:
        file.write(code)

    try:
        subprocess.run(["g++", "temp3.cpp", "-o", "temp3.out"], check=True)
        result = subprocess.run(["./temp3.out"], capture_output=True, text=True, check=True)
        return result.stdout
    except subprocess.CalledProcessError as e:
        return f"Error: {e.stderr}"

subscriber = mqtt.Client("Child_Node_Three")
